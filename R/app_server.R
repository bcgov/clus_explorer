...
app_server <- function(input, output, session) {
 ...
 "');"
    ...
    list(
      harvest = data.table(getTableQuery(
        paste0(
          "SELECT * FROM ",
          schema_scenarios$schema,
          ".harvest where scenario IN ('",
          paste(schema_scenarios$scenario, sep =  "' '", collapse = "', '"),
          "');"
        )
      )),
      growingstock = data.table(getTableQuery(
        paste0(
          "SELECT scenario, timeperiod, sum(m_gs) as growingstock FROM ",
          schema_scenarios$schema,
          ".growingstock where scenario IN ('",
          paste(schema_scenarios$scenario, sep =  "' '", collapse = "', '"),
          "') group by scenario, timeperiod;"
        )
      )),
      rsf = data.table(getTableQuery(
        paste0(
          "SELECT * FROM ",
          schema_scenarios$schema,
          ".rsf where scenario IN ('",
          paste(schema_scenarios$scenario, sep =  "' '", collapse = "', '"),
...
ON (foo1.scenario = foo2.scenario) )"
  ...
  output$radar <- renderPlotly ({
    ...
    radarNames <-
      paste(c(names(radarList())[2:length(radarList())], names(radarList())[2]), collapse = "', '")
    radarData <-
      radarList()[, data := paste(.SD, collapse = ',')  , .SDcols = c(names(radarList())[2:length(radarList())], names(radarList())[2]), by = scenario]
